 const data = {
    "message": "Please parse the 'data' array, which is an array of objects, having a 'name' field. Count the number of words matching 'version' in the name string (case-insensitive) and POST the result to /challenges with payload {count: <value>}",
    "data": [
        {
            "id": "AAL",
            "identifiers": [
        ...
        },
        {
            ...
            "name": "Jabber Open Source License, version -0.1, version -0.2",
            "other_names": [],
            "superseded_by": null,
            "keywords": [
                "discouraged",
...
// "Please parse the 'data' array, which is an array of objects, having a 'name' field. Count the number of words matching 'version' in the name string (case-insensitive) and POST the result to /challenges with payload {count: <value>}",


function countWordVersion(data) {
    let count = 0;
    for (let i = 0; i < data.length; i++) {
        let name = data[i].name.toLowerCase();
        let nameArray = name.split(' ');
        for (let j = 0; j < nameArray.length; j++) {
            if (nameArray[j] === 'version') {
                count++
            }
        }
    }
